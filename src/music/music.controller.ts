...
export class MusicController {
  ...
  @ApiOperation({
    summary:
      'Upload a new music into the system, for this user must have CREATOR role (USER)',
  })
  @Post('/upload')
  ...
  @ApiOperation({
    summary: 'Get all musics uploaded by the logged in user (USER)',
  })
  @Get('/own')
  async findOwnMusic(@Query() filter: IFilter, @Request() req: Req) {
    const user = req.user as User;
    const paginationConfig = getPaginationConfig(filter);
    const [data, count] = await this.musicService.find(
      paginationConfig.take,
  ...
  async getLikedMusics(@Query() filter: IFilter, @Request() req: Req) {
    const user = req.user as User;
    const paginationConfig = getPaginationConfig(filter);
    const [musics, total] = await this.musicService.findAllLikedByUser(
      paginationConfig.take,
      paginationConfig.skip,
      user,
      filter.query,
  ...
  async findOne(@Param('id') id: string) {
    ...
    const musicFile = music.media.find(
      (media) => media.type === MediaType.MUSIC,
    );
    const thumbnail = music.media.find(
  ...
  async remove(@Param('id') id: string, @Request() req: Req) {
    ...
    if (user.id !== music.uploadedBy.id) {
